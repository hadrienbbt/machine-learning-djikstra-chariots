...
namespace Classification_Kohonen
{
	static class ControlerData
	{
		...
		public static List<Observation> nouvelOrdre(List<List<double>> lvecteursapprentissage)
		{
			List<Observation> pioche = ControlerData.ObservationFactory(lvecteursapprentissage);
		...
		public static List<Observation> ObservationFactory(List<List<double>> listeVect) // 3000 elem
		{
			List<Observation> res = new List<Observation>();

			for (int i = 0; i < listeVect.Count; i++) res.Add(new Observation(listeVect[i][0], listeVect[i][1]));

			return res;
		}

		public static List<Observation> ObservationFactory(List<List<double>> listeVect, int numEch) // 3 échantillons possibles : 0,1,2
		{
			List<Observation> res = new List<Observation>();

			for (int i = numEch * 1000; i < numEch * 1000 + 1000; i++) res.Add(new Observation(listeVect[i][0], listeVect[i][1]));
